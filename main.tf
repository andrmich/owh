...
resource "aws_alb_target_group" "simple_vm" {
  name     = var.tags.Topic
  port     = 8080
  protocol = "HTTP"
  vpc_id   = data.terraform_remote_state.stage_resources.outputs.vpc_id
...
resource "aws_alb_target_group_attachment" "simple_vm" {
  target_group_arn = aws_alb_target_group.simple_vm.arn
  target_id = aws_instance.simple_vm.id

}

resource "aws_alb_listener_rule" "simple_vm_rule" {
